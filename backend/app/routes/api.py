...
def ordenar_servicios(servicios: List[dict], sort_by: str = "nombre", order: str = "asc") -> List[dict]:
    """
    ...
    if sort_by == "distancia":
        ...
        return sorted(
            servicios,
            key=lambda x: (x.get("distancia_aproximada") is None, x.get("distancia_aproximada", float('inf'))),
            reverse=reverse
        )
    else:
...
def ordenar_interpretes(interpretes: List[dict], sort_by: str = "nombre", order: str = "asc") -> List[dict]:
    """
    ...
    if sort_by == "experiencia":
        ...
        return sorted(
            interpretes,
            key=lambda x: x.get("años_experiencia", 0),
            reverse=reverse
        )
    else:
...
def get_servicios(
    ...
    subcategoria: Optional[str] = Query(None, description="Filtra por subcategoría específica"),
    zona: Optional[str] = Query(None, description="Filtra por zona de Medellín"),
    tiene_interprete: Optional[bool] = Query(None, description="Filtra servicios con intérprete LSC"),
    sort_by: Optional[str] = Query("nombre", description="Ordenar por: nombre, zona, distancia"),
    order: Optional[str] = Query("asc", description="Orden: asc o desc"),
    page: Optional[int] = Query(None, ge=1, description="Número de página (inicia paginación)"),
    limit: Optional[int] = Query(10, ge=1, le=50, description="Items por página (máx 50)")
):
    """
...
def get_servicios_por_zona(
    ...
    tiene_interprete: Optional[bool] = Query(None, description="Filtro adicional por intérprete LSC"),
    sort_by: Optional[str] = Query("nombre", description="Ordenar por: nombre, zona, distancia"),
    order: Optional[str] = Query("asc", description="Orden: asc o desc"),
    page: Optional[int] = Query(None, ge=1, description="Número de página"),
    limit: Optional[int] = Query(10, ge=1, le=50, description="Items por página")
...
def get_interpretes(
    ...
    disponibilidad: Optional[str] = Query(None, description="Filtra por tipo de disponibilidad"),
    sort_by: Optional[str] = Query("nombre", description="Ordenar por: nombre, experiencia"),
    order: Optional[str] = Query("asc", description="Orden: asc o desc"),
    page: Optional[int] = Query(None, ge=1, description="Número de página"),
    limit: Optional[int] = Query(10, ge=1, le=50, description="Items por página")
):
...
def get_interpretes_por_disponibilidad(
    tipo: str = Query(..., description="Tipo: inmediata, tiempo_completo, por_horas, fines_semana"),
    especialidad: Optional[str] = Query(None, description="Filtro adicional por especialidad"),
